...
var catalog = map[string]map[Lang]string{
	...
	"login.no_pty":      {JA: "** 注意: 端末(pty)無しで接続しています。1キー操作に Enter が必要（rogue・メニュー）／全画面のノート編集が崩れる／Ctrl-C で切断される、などが起きます。快適に使うには `ssh -t -p 2222 %s@ホスト` のように -t を付けて繋ぎ直してください。 **", EN: "** Notice: connected without a terminal (pty). Single-key controls need Enter (rogue, menus); the full-screen note editor renders incorrectly; Ctrl-C may disconnect you. For a proper session reconnect with -t, e.g. `ssh -t -p 2222 %s@host`. **"},
