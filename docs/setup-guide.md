...
| `who` / `log` | 在室一覧 / アクセスログ |
| `rogue` | 内蔵ゲーム ローグ（MAIN→OTHERS の `[8] rogue`）。`rogue -s` で成績 |
| `off` | ログアウト |

発言を打つ場（chat / talk）では素の文字が発話になるため、操作は `/` 付き
（`/i` 一覧・`/q` 退出・`/?` ヘルプ）です。

内蔵ゲーム **ローグ**（`rogue`）は、洞窟を潜って地下 26 階のイェンダーの魔除けを
持ち帰る全画面ゲームです（原典 Rogue 系のクローンを Go でフルスクラッチ実装した
Wick 追加コマンド）。セーブは 1 人 1 本で、続きから始めると消えます。成績は
`rogue -s` のスコア表に残ります。全画面表示のため **端末は pty あり・80×24 以上**が
必要です（`ssh -t …` で接続。terminal コマンドでサイズ設定）。ゲーム内キーは起動後の
`?` または `rogue -?`（`data/help/rogue.usg`）を参照してください。
